...
years.each do |year|
  File.open("/Users/alexgaziev/ids#{year}.list", 'w+') do |file|
    ...
    while all >= skip
      ...
      rd = open("http://www.imdb.com/List?tvm=off&&tv=off&&ep=off&&vid=off&&year=#{year}&&skip=#{skip}", 'User-Agent' => 
        'Mozilla/5.0 (X11; U; Linux i686; en-US; rv:1.9.1.2) Gecko/20090803 Ubuntu/9.04 (jaunty) Shiretoko/3.5.2').read
    end
  end
end
